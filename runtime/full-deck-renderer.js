...
const renderSlide = (slide, index, total) => `<section class="slide primitive-${attr(slide.composition.primitive)} motion-root variant-${attr(slide.composition.variant)}" id="${attr(slide.id)}" data-slide-id="${attr(slide.id)}" data-primitive="${attr(slide.composition.primitive)}">${primitiveRenderers[slide.composition.primitive](slide, index, total)}</section>`;
