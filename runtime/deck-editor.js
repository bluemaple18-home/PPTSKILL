...
const tag=q('#deck-spec');if(!tag)return;let spec=JSON.parse(tag.textContent);const safeStyle=clone(spec.style);let currentId=spec.slides[0]?.id||'';let editMode=false;let editingComponentId='',composingText=null;
...
document.addEventListener('click',e=>{const slide=e.target.closest?.('.slide');if(slide)select(slide.dataset.slideId);const action=e.target.closest?.('[data-action]')?.dataset.action;if(!action)return;if(action==='edit')setEdit(!editMode);if(action==='edit-component')openComponentEditor();if(action==='apply-component')applyComponentEditor();if(action==='cancel-component')q('[data-component-dialog]').close();if(action==='move-up')move(-1);if(action==='move-down')move(1);if(action==='duplicate')duplicate();if(action==='delete')remove();if(action==='save')download()});
const textEventRoot=document.body?.addEventListener?document.body:document;
textEventRoot.addEventListener('dblclick',e=>{const target=resolveDirectTextTarget(e.target);if(!target)return;select(target.slideId);setEdit(true);target.element.contentEditable='true';target.element.focus?.();e.preventDefault?.()});
