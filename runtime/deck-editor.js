...
import { COMPONENT_GEOMETRY, validateComponentGeometry, updateComponentGeometry, buildComponentGeometryRuntime } from './component-geometry.js';
...
.pptskill-editor{max-width:calc(100vw - 36px);flex-wrap:wrap}body[data-editor-mode="layout"] .pptskill-editor{opacity:1}body[data-editor-mode="layout"] .pptskill-editor>[data-action="save"],body[data-editor-mode="layout"] .pptskill-editor>[data-action="delete"],body[data-editor-mode="layout"] .pptskill-editor>[data-editor-status],body[data-editor-mode="layout"] .pptskill-editor>[data-action="initialize-layout"]:not([hidden]){display:inline-flex}.pptskill-editor>[hidden]{display:none!important}body[data-editor-mode="layout"] .slide [data-editor-selected="true"]{outline:2px solid #2f82ff;outline-offset:2px}body[data-editor-mode="layout"] .slide [data-pptskill-element-id]{user-select:none}body[data-editor-mode="layout"] .moveable-control-box{z-index:9999}
@media print{[data-pptskill-editor-chrome],.moveable-control-box{display:none!important}.pptskill-editor{display:none!important}.slide[data-editor-selected="true"]{box-shadow:none}}
`;

export const buildDeckEditorMarkup = () => `<nav class="pptskill-editor" data-pptskill-editor aria-label="簡報編輯器"><button type="button" data-action="edit">編輯文字</button><button type="button" data-action="layout" aria-pressed="false">編輯版面</button><button type="button" data-action="initialize-layout" hidden>套用手動版面</button><button type="button" data-action="edit-component">編輯元件</button><button type="button" data-action="move-up">上移</button><button type="button" data-action="move-down">下移</button><button type="button" data-action="duplicate">複製</button><button type="button" data-action="delete">刪除</button><label for="pptskill-image-input">替換圖片</label><input class="pptskill-editor__file" id="pptskill-image-input" type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml"><button type="button" data-action="save">另存 HTML</button><span class="pptskill-editor__status" data-editor-status aria-live="polite">可直接播放</span></nav><dialog class="pptskill-component-dialog" data-component-dialog><strong>編輯目前頁面的支援元件</strong><p>只接受 DeckSpec allowlist 內的 text、image、table、chart 或 public citation。</p><textarea data-component-json spellcheck="false"></textarea><menu><button type="button" data-action="cancel-component">取消</button><button type="button" data-action="apply-component">套用</button></menu></dialog>`;
...
const remove=()=>{layout?.clearSelection();if(spec.slides.length===1){status('至少保留一頁');return}const i=currentIndex(),node=currentNode(),removed=spec.slides.splice(i,1)[0];if(Array.isArray(spec.claims))spec.claims=spec.claims.map(c=>({...c,slideIds:c.slideIds.filter(id=>id!==removed.id)})).filter(c=>c.slideIds.length);const next=node.nextElementSibling||node.previousElementSibling,background=q('[data-pptskill-background-layer]',node);if(background)window.PPTSKILLBackground?.destroy(background);node.remove();select(next.dataset.slideId);renumber();next.scrollIntoView({behavior:'smooth',block:'start'});status('已刪除投影片')};
...
const executeOperation=request=>{const o=validateOperationRequest(request);if(!operationDescriptors[o.operation])throw new Error('不支援的 operation。');const slide=spec.slides.find(s=>s.id===o.target.slideId);if(!slide)throw new Error('找不到 slide。');const identities=resolveSlideElementIdentities(slide),pointIndex=identities.keyPoints.indexOf(o.target.elementId),role=o.target.elementId===roleElementIds.title?'title':o.target.elementId===roleElementIds.subtitle?'subtitle':pointIndex>=0?'keyPoint':identities.components.includes(o.target.elementId)?'component':null;if(!role)throw new Error('找不到 element。');if(!operationDescriptors[o.operation].allowedTargetRoles.includes(role))throw new Error('operation 不支援 target role：'+role);const before=clone(spec);try{if(role==='component')updateComponentGeometry(slide,slide.content.components[identities.components.indexOf(o.target.elementId)].id,o.operation,o.value);else if(role==='title'||role==='subtitle')slide.content[role]=o.value;else slide.content.keyPoints[pointIndex]=o.value;spec=clean();const cleanedSlide=spec.slides.find(s=>s.id===o.target.slideId),root=q('.slide[data-slide-id="'+CSS.escape(o.target.slideId)+'"]'),element=q('[data-pptskill-element-id="'+CSS.escape(o.target.elementId)+'"]',root);if(role==='component'){projectComponentGeometry(document,spec)}else if(role==='keyPoint'&&element?.matches('.metric-cards article'))updateMetricPoint(o.target.slideId,cleanedSlide,pointIndex,o.value);else if(element)element.textContent=o.value;return spec}catch(error){spec=before;throw error}};
...
layout=mountComponentInteraction({document,window,getSpec:()=>spec,resolveIdentities:resolveSlideElementIdentities,executeOperation,project:()=>projectComponentGeometry(document,spec),notify:status,setTextMode:setEdit,selectSlide:select});
