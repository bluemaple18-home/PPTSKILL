...
const previewEvent=(handler,render=true)=>event=>{if(mutationBlocked())return false;mutationOwner=true;try{const result=handler(event);if(render)refreshHistoryControls(true);return result}finally{mutationOwner=false}};
const history=createEditorHistory();
...
const setSelectedImageFit=fit=>{if(cropDialog?.isOpen()||pendingTextInsertion||imagePickerOpen||!['contain','cover'].includes(fit))return;const target=selectedImageTarget();if(!target)return;layout.cancel('image-fit');const component=selectedImageComponent(target);if((component.fit||'contain')===fit)return;try{mutate(()=>executeOperationInternal({operation:'replace-asset',target,value:{dataUri:component.dataUri,fit}}))}catch(error){status(error.message)}};
...
const commitTextElement=(element,afterCommit=()=>{})=>{const target=resolveDirectTextTarget(element);if(!target)return false;if(composingText===target.element)throw new Error('IME 組字尚未完成，暫不可提交或匯出。');const value=target.element.textContent.trim();if(value===target.canonical())return false;executeOperationInternal({operation:'edit-text',target:{slideId:target.slideId,elementId:target.elementId},value},afterCommit);return true};
...
const syncText=()=>{if(composingText)throw new Error('IME 組字尚未完成，暫不可提交或匯出。');directTextElements().forEach(element=>commitTextElement(element))};
...
const runMutation=(work,projection=[],afterRollback)=>{assertMutationAvailable();mutationOwner=true;try{return mutate(work,projection,afterRollback)}finally{mutationOwner=false}};
const mutate=(work,projection=[],afterRollback)=>{
let saved;
try{
...
textEventRoot.addEventListener('focusin',previewEvent(e=>{const target=resolveDirectTextTarget(e.target);if(!editMode)return;if(target){select(target.slideId);setTypographyTarget(target)}else if(!e.target.closest?.('.pptskill-editor,[data-pptskill-typography-toolbar]'))clearTypographyTarget()}));
textEventRoot.addEventListener('compositionstart',previewEvent(e=>{const target=resolveDirectTextTarget(e.target);if(editMode&&target)composingText=target.element}));
textEventRoot.addEventListener('compositionend',previewEvent(e=>{const target=resolveDirectTextTarget(e.target);if(!target||composingText!==target.element)return;composingText=null;if(!commitTextElement(target.element,()=>status('文字已更新'))){status('文字已更新');refreshHistoryControls(true)}},false));
textEventRoot.addEventListener('focusout',previewEvent(e=>{const target=resolveDirectTextTarget(e.target);if(!editMode||!target||composingText===target.element)return;if(!commitTextElement(target.element))refreshHistoryControls(true)},false));
textEventRoot.addEventListener('keydown',previewEvent(e=>{if(e.key!=='Escape'||e.isComposing)return;const target=resolveDirectTextTarget(e.target);if(!editMode||!target||composingText===target.element)return;restoreTextElement(target.element);e.preventDefault();e.stopImmediatePropagation?.();status('已取消文字變更')}));
...
qa('.slide').forEach(slide=>slide.addEventListener('focusin',previewEvent(()=>select(slide.dataset.slideId))));select(currentId);document.body.dataset.editorMode='play';
