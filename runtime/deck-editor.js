...
const move=delta=>{layout?.clearSelection();syncText();const i=currentIndex(),to=i+delta;if(i<0||to<0||to>=spec.slides.length)return;const node=currentNode(),other=delta<0?node.previousElementSibling:node.nextElementSibling,parent=node.parentNode,next=node.nextSibling,folios=qa('[data-editor-folio]').map(el=>[el,el.textContent]);let swapped=false;try{if(delta<0)parent.insertBefore(node,other);else parent.insertBefore(other,node);[spec.slides[i],spec.slides[to]]=[spec.slides[to],spec.slides[i]];swapped=true;renumber();status('已調整順序');revision++;history.clear()}catch(error){if(swapped)[spec.slides[i],spec.slides[to]]=[spec.slides[to],spec.slides[i]];try{if(node.parentNode===parent&&node.nextSibling!==next)parent.insertBefore(node,next)}catch{}for(const [el,value]of folios)try{if(el.textContent!==value)el.textContent=value}catch{}throw error}};
...
const historyBusy=()=>Boolean(pasteInputOwned(document.activeElement)||document.activeElement?.closest?.('dialog')||composingText||textInsertionComposing||pendingTextInsertion||textInsertionBusy||insertionBusy||imagePickerOpen||pendingAssetOperations||deletionInProgress||cropCommitInProgress||groupCommitInProgress||cropDialog?.isOpen()||q('[data-component-dialog]')?.open||layout?.getState().gesturing);
const historyMode=()=>Boolean(editMode||layout?.getState().enabled);
const refreshHistoryControls=()=>{const state=history.state(),blocked=!historyMode()||historyBusy()||spec.slides.filter(s=>s.id===currentId).length!==1||qa('.slide').filter(s=>s.dataset.slideId===currentId).length!==1;for(const [direction,enabled]of [['undo',state.canUndo],['redo',state.canRedo]]){const button=q('[data-action="'+direction+'"]');if(button){button.disabled=blocked||!enabled;button.title=state.oversized?'歷史超過 64 MiB，無法保留':button.disabled?'目前無法'+(direction==='undo'?'復原':'重做'):''}}};
