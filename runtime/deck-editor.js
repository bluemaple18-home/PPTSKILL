...
import { COMPONENT_GEOMETRY, COMPONENT_ALIGNMENT_VALUES, validateComponentGeometry, updateComponentGeometry, alignComponentGeometries, buildComponentGeometryRuntime } from './component-geometry.js';
...
const editTextAllowedTargetRoles = Object.freeze(['title', 'subtitle', 'keyPoint']);
...
const validateOperationRequest = (request) => {
  ...
  if (request.operation === 'align-selection') {
    if (!hasExactKeys(request.target, ['slideId', 'elementIds'])) throw new Error('align-selection target 欄位必須恰為 slideId、elementIds。');
    if (typeof request.target.slideId !== 'string' || request.target.slideId.length < 1) throw new Error('operation target slideId 格式非法。');
    const ids = request.target.elementIds;
    if (!Array.isArray(ids) || ids.length < 2 || ids.some(id => typeof id !== 'string' || !operationIdPattern.test(id))
      || new Set(ids).size !== ids.length) throw new Error('align-selection elementIds 必須是至少兩個唯一合法 ID。');
    if (!hasExactKeys(request.value, ['alignment']) || !COMPONENT_ALIGNMENT_VALUES.includes(request.value.alignment)) throw new Error('align-selection alignment 不支援。');
    return request;
  }
...
export const OPERATION_DESCRIPTORS = deepFreeze({
  'edit-text': {
    ...
    mutates: ['content.text'],
    preserves: ['composition', 'style', 'motion', 'background', 'components', 'otherSlides'],
    destructive: false,
    confirmation: 'none',
    undoable: true,
...
export function createDeckEditor(input) {
  ...
  const executeOperation = (request) => {
    ...
    if (operation === 'align-selection') {
      const componentIds = target.elementIds.map(elementId => {
        const index = identities.components.indexOf(elementId);
        if (index < 0) throw new Error(`align-selection 找不到 component element：${elementId}`);
        return slide.content.components[index].id;
      });
      alignComponentGeometries(slide, componentIds, value.alignment);
    ...
    if (role === 'component') updateComponentGeometry(slide, slide.content.components[identities.components.indexOf(target.elementId)].id, operation, value);
...
.pptskill-editor{max-width:calc(100vw - 36px);flex-wrap:wrap}body[data-editor-mode="layout"] .pptskill-editor{opacity:1}body[data-editor-mode="layout"] .pptskill-editor>[data-action="snap-layout"],body[data-editor-mode="layout"] .pptskill-editor>[data-action="save"],body[data-editor-mode="layout"] .pptskill-editor>[data-action="delete"],body[data-editor-mode="layout"] .pptskill-editor>[data-editor-status],body[data-editor-mode="layout"] .pptskill-editor>[data-action="initialize-layout"]:not([hidden]),body[data-editor-mode="layout"] .pptskill-editor>[data-pptskill-context-toolbar]:not([hidden]){display:inline-flex}.pptskill-editor>[data-pptskill-context-toolbar]{align-items:center;gap:4px}.pptskill-editor>[hidden]{display:none!important}body[data-editor-mode="layout"] .slide [data-editor-selected="true"]{outline:2px solid #2f82ff;outline-offset:2px}body[data-editor-mode="layout"] .slide [data-pptskill-element-id]{user-select:none}body[data-editor-mode="layout"] .moveable-control-box{z-index:9999}
...
const validateOperationRequest=request=>{if(!hasExactKeys(request,['operation','target','value']))throw new Error('operation payload 欄位必須恰為 operation、target、value。');if(typeof request.operation!=='string'||!idPattern.test(request.operation))throw new Error('operation ID 格式非法。');if(request.operation==='align-selection'){if(!hasExactKeys(request.target,['slideId','elementIds']))throw new Error('align-selection target 欄位必須恰為 slideId、elementIds。');if(typeof request.target.slideId!=='string'||request.target.slideId.length<1)throw new Error('operation target slideId 格式非法。');const ids=request.target.elementIds;if(!Array.isArray(ids)||ids.length<2||ids.some(id=>typeof id!=='string'||!idPattern.test(id))||new Set(ids).size!==ids.length)throw new Error('align-selection elementIds 必須是至少兩個唯一合法 ID。');if(!hasExactKeys(request.value,['alignment'])||!COMPONENT_ALIGNMENT_VALUES.includes(request.value.alignment))throw new Error('align-selection alignment 不支援。');return request}if(!hasExactKeys(request.target,['slideId','elementId']))throw new Error('operation target 欄位必須恰為 slideId、elementId。');if(typeof request.target.slideId!=='string'||request.target.slideId.length<1)throw new Error('operation target slideId 格式非法。');if(typeof request.target.elementId!=='string'||!idPattern.test(request.target.elementId))throw new Error('operation target elementId 格式非法。');if(request.operation==='edit-text'&&typeof request.value!=='string')throw new Error('operation value 必須是 string。');if(request.operation==='move-element')validateComponentGeometry(request.value,['x','y']);if(request.operation==='resize-element')validateComponentGeometry(request.value,['width','height']);return request};
...
const select=id=>{if(currentId!==id)layout?.clearSelection();currentId=id;qa('.slide').forEach(s=>s.dataset.editorSelected=String(s.dataset.slideId===id));};
const pointTarget=(id,i)=>'slides.'+id+'.content.keyPoints.'+i;
const syncText=()=>qa('[data-edit-kind="text"]').forEach(el=>{const t=el.dataset.editTarget||'',slide=spec.slides.find(s=>t.startsWith('slides.'+s.id+'.content.'));if(!slide)return;const field=t.slice(('slides.'+slide.id+'.content.').length);if(field==='title'||field==='subtitle'){setValue(slide.content,field,el.textContent.trim());return}const p=/^keyPoints\.(\d+)$/.exec(field);if(p&&slide.content.keyPoints[Number(p[1])]!==undefined){setValue(slide.content.keyPoints,Number(p[1]),el.textContent.trim());return}if(field.startsWith('components.')){const id=field.slice(11),component=slide.content.components.find(x=>x.id===id);if(component?.type==='text')setValue(component,'text',el.textContent.trim());if(component?.type==='citation')setValue(component,'label',el.textContent.trim())}});
...
const setEdit=on=>{if(on&&layout?.getState().enabled)layout.setMode(false);editMode=on;document.body.dataset.editorMode=on?'edit':'play';qa('[data-edit-kind="text"]').forEach(el=>el.contentEditable=String(on));q('[data-action="edit"]').textContent=on?'完成編輯':'編輯文字';status(on?'正在編輯':'變更已保留')};
...
const executeOperation=request=>{const o=validateOperationRequest(request);if(!operationDescriptors[o.operation])throw new Error('不支援的 operation。');const slide=spec.slides.find(s=>s.id===o.target.slideId);if(!slide)throw new Error('找不到 slide。');const identities=resolveSlideElementIdentities(slide);if(o.operation==='align-selection'){const before=clone(spec);try{const componentIds=o.target.elementIds.map(elementId=>{const index=identities.components.indexOf(elementId);if(index<0)throw new Error('align-selection 找不到 component element：'+elementId);return slide.content.components[index].id});alignComponentGeometries(slide,componentIds,o.value.alignment);spec=clean();projectComponentGeometry(document,spec);markChanged(before,spec);return spec}catch(error){spec=before;throw error}}const pointIndex=identities.keyPoints.indexOf(o.target.elementId),role=o.target.elementId===roleElementIds.title?'title':o.target.elementId===roleElementIds.subtitle?'subtitle':pointIndex>=0?'keyPoint':identities.components.includes(o.target.elementId)?'component':null;if(!role)throw new Error('找不到 element。');if(!operationDescriptors[o.operation].allowedTargetRoles.includes(role))throw new Error('operation 不支援 target role：'+role);const before=clone(spec);try{if(role==='component')updateComponentGeometry(slide,slide.content.components[identities.components.indexOf(o.target.elementId)].id,o.operation,o.value);else if(role==='title'||role==='subtitle')slide.content[role]=o.value;else slide.content.keyPoints[pointIndex]=o.value;spec=clean();const cleanedSlide=spec.slides.find(s=>s.id===o.target.slideId),root=q('.slide[data-slide-id="'+CSS.escape(o.target.slideId)+'"]'),element=q('[data-pptskill-element-id="'+CSS.escape(o.target.elementId)+'"]',root);if(role==='component'){projectComponentGeometry(document,spec)}else if(role==='keyPoint'&&element?.matches('.metric-cards article'))updateMetricPoint(o.target.slideId,cleanedSlide,pointIndex,o.value);else if(element)element.textContent=o.value;markChanged(before,spec);return spec}catch(error){spec=before;throw error}};
...
projectComponentGeometry(document,spec);
...
layout=mountComponentInteraction({document,window,getSpec:()=>spec,getRevision:()=>revision,resolveIdentities:resolveSlideElementIdentities,executeOperation,project:()=>projectComponentGeometry(document,spec),notify:status,setTextMode:setEdit,selectSlide:select});
