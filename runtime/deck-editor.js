...
let deletionInProgress=false,pendingAssetOperations=0;
...
if(node.dataset.editKind||images.length!==1||node.children.length!==1||img.parentNode!==node||node.textContent.trim()||img.getAttribute('src')!==component.dataUri||img.getAttribute('alt')!==component.alt||img.style.getPropertyValue('object-fit')!==(component.fit||'contain'))throw new Error('delete-element image DOM 與 canonical 失配。');
}
...
layout?.cancel('delete-element');layout?.clearSelection();refreshSelectedImage('refresh');return spec
...
executeOperation({operation:'replace-asset',target,value:{dataUri:result.dataUri}});status(message);return result}finally{pendingAssetOperations--}};
