...
const restoreHistoryControls=saved=>{for(const item of saved)if(item){const [button,disabled,title]=item;try{button.disabled=disabled;button.title=title}catch{}}};
...
const executeOperation=request=>{const o=validateOperationRequest(request);const before=clone(spec),revisionBefore=revision,checkpoint=captureHistoryDom(),historyBefore=history.checkpoint(),controls=captureHistoryControls(),selectionBefore=layout?.getSelectionState(),statusBefore=q('[data-editor-status]')?.textContent;try{const result=executeOperationCore(request);if(!sameCanonicalValue(before,spec)){history.record(before,clone(spec),operationDescriptors[o.operation]?.undoable===true);refreshHistoryControls()}return result}catch(error){spec=before;revision=revisionBefore;history.restore(historyBefore);const changed=historyDomChanged(checkpoint);if(changed)restoreHistoryDom(checkpoint);try{if(changed)layout?.restoreProjection?.();if(selectionBefore&&JSON.stringify(layout?.getSelectionState())!==JSON.stringify(selectionBefore))layout?.restoreSelection?.(selectionBefore)}catch{}restoreHistoryControls(controls);if(statusBefore!==undefined)try{status(statusBefore)}catch{}throw error}}
