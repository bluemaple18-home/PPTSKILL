import { componentElementId, pointElementId, ROLE_ELEMENT_IDS, sanitizeDeckSpec, validateDeckSpec } from './deck-spec.js';
...
const validateOperationRequest = (request) => {
  ...
  if (typeof request.target.slideId !== 'string' || request.target.slideId.length < 1 || request.target.slideId.length > 200) throw new Error('operation target slideId 格式非法。');
  if (!operationIdPattern.test(request.target.elementId)) throw new Error('operation target elementId 格式非法。');
...
export const OPERATION_DESCRIPTORS = Object.freeze({
  'edit-text': Object.freeze({
    ...
    allowedTargetRoles: ['title', 'subtitle', 'keyPoint'],
...
export function createDeckEditor(input) {
  ...
  const executeOperation = (request) => {
    ...
    else if (slide.content.keyPointIds.some((id) => pointElementId(id) === target.elementId)) role = 'keyPoint';
    else if (slide.content.components.some(({ id }) => componentElementId(id) === target.elementId)) role = 'component';
    ...
    if (role === 'title' || role === 'subtitle') slide.content[role] = value;
    else slide.content.keyPoints[slide.content.keyPointIds.findIndex((id) => pointElementId(id) === target.elementId)] = value;
    return commit(candidate);
  };

  const editor = {
    getSpec: () => clone(spec),
    operationDescriptors: clone(OPERATION_DESCRIPTORS),
    executeOperation,
    ...
    editKeyPoint(slideId, index, value) {
      ...
      return executeOperation({ operation: 'edit-text', target: { slideId, elementId: pointElementId(slide.content.keyPointIds[index]) }, value: String(value) });
    },
...
export const buildDeckEditorRuntimeScript = () => {
...
const validateOperationRequest=request=>{if(!hasExactKeys(request,['operation','target','value']))throw new Error('operation payload 欄位必須恰為 operation、target、value。');if(typeof request.operation!=='string'||!idPattern.test(request.operation))throw new Error('operation ID 格式非法。');if(!hasExactKeys(request.target,['slideId','elementId']))throw new Error('operation target 欄位必須恰為 slideId、elementId。');if(typeof request.target.slideId!=='string'||request.target.slideId.length<1||request.target.slideId.length>200)throw new Error('operation target slideId 格式非法。');if(!idPattern.test(request.target.elementId))throw new Error('operation target elementId 格式非法。');if(typeof request.value!=='string')throw new Error('operation value 必須是 string。');return request};
...
const duplicate=()=>{syncText();if(spec.slides.length>=15){status('最多 15 頁');return}const i=currentIndex(),source=spec.slides[i],copy=clone(source),id=uniqueId(source.id);copy.id=id;spec.slides.splice(i+1,0,copy);for(const claim of spec.claims||[])if(claim.slideIds.includes(source.id))claim.slideIds.push(id);const node=currentNode(),next=node.cloneNode(true);retarget(next,source.id,id);const background=q('[data-pptskill-background-layer]',next);if(background){background.replaceChildren();background.removeAttribute('style');background.dataset.backgroundState='static'}node.after(next);if(background)window.PPTSKILLBackground?.init(background);select(id);renumber();next.scrollIntoView({behavior:'smooth',block:'start'});setEdit(editMode);status('已複製投影片')};
const remove=()=>{if(spec.slides.length===1){status('至少保留一頁');return}const i=currentIndex(),node=currentNode(),removed=spec.slides.splice(i,1)[0];if(Array.isArray(spec.claims))spec.claims=spec.claims.map(c=>({...c,slideIds:c.slideIds.filter(id=>id!==removed.id)})).filter(c=>c.slideIds.length);const next=node.nextElementSibling||node.previousElementSibling,background=q('[data-pptskill-background-layer]',node);if(background)window.PPTSKILLBackground?.destroy(background);node.remove();select(next.dataset.slideId);renumber();next.scrollIntoView({behavior:'smooth',block:'start'});status('已刪除投影片')};
const esc=v=>String(v??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
const renderComponent=c=>{const target='slides.'+currentId+'.content.components.'+c.id,identity=' data-pptskill-element-id="'+esc(componentElementId(c.id))+'"';if(c.type==='image')return '<figure class="asset image-asset" data-effect-role="image"'+identity+' data-edit-target="'+esc(target)+'"><img src="'+esc(c.dataUri)+'" alt="'+esc(c.alt)+'" style="object-fit:'+(c.fit||'contain')+'"></figure>';if(c.type==='text')return '<blockquote class="asset text-asset" data-effect-role="visualAnchor" data-edit-kind="text"'+identity+' data-edit-target="'+esc(target)+'">'+esc(c.text)+'</blockquote>';if(c.type==='citation')return '<p class="asset citation-asset" data-edit-kind="text"'+identity+' data-edit-target="'+esc(target)+'">'+(c.url?'<a href="'+esc(c.url)+'">'+esc(c.label)+'</a>':esc(c.label))+'</p>';if(c.type==='table')return '<div class="asset table-asset" data-effect-role="diagram"'+identity+' data-edit-target="'+esc(target)+'"><table><thead><tr>'+c.headers.map(h=>'<th>'+esc(h)+'</th>').join('')+'</tr></thead><tbody>'+c.rows.map(r=>'<tr>'+r.map(v=>'<td>'+esc(v)+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>';if(c.type==='chart'){const max=Math.max(1,...c.series.flatMap(s=>s.values));return '<div class="asset chart-asset" data-effect-role="diagram"'+identity+' data-edit-target="'+esc(target)+'">'+c.series.map(s=>'<section><b>'+esc(s.name)+'</b>'+s.values.map((v,i)=>'<div class="bar-row"><span>'+esc(c.labels[i]||'')+'</span><i style="--bar:'+Math.max(0,Math.min(100,Math.round(v/max*100)))+'%"></i><em>'+esc(v)+'</em></div>').join('')+'</section>').join('')+'</div>'}return ''};
...
const executeOperation=request=>{const o=validateOperationRequest(request);if(!operationDescriptors[o.operation])throw new Error('不支援的 operation。');const slide=spec.slides.find(s=>s.id===o.target.slideId);if(!slide)throw new Error('找不到 slide。');const pointIndex=slide.content.keyPointIds.findIndex(id=>pointElementId(id)===o.target.elementId),role=o.target.elementId===roleElementIds.title?'title':o.target.elementId===roleElementIds.subtitle?'subtitle':pointIndex>=0?'keyPoint':slide.content.components.some(c=>componentElementId(c.id)===o.target.elementId)?'component':null;if(!role)throw new Error('找不到 element。');if(!operationDescriptors[o.operation].allowedTargetRoles.includes(role))throw new Error('operation 不支援 target role：'+role);syncText();const before=clone(spec);try{if(role==='title'||role==='subtitle')slide.content[role]=o.value;else slide.content.keyPoints[pointIndex]=o.value;spec=clean();const cleanedSlide=spec.slides.find(s=>s.id===o.target.slideId),root=q('.slide[data-slide-id="'+CSS.escape(o.target.slideId)+'"]'),element=q('[data-pptskill-element-id="'+CSS.escape(o.target.elementId)+'"]',root);if(role==='keyPoint'&&element?.matches('.metric-cards article'))updateMetricPoint(o.target.slideId,cleanedSlide,pointIndex,o.value);else if(element)element.textContent=o.value;return spec}catch(error){spec=before;throw error}};
const applyPatch=p=>{if(!p||p.slideId!==currentId)throw new Error('patch 必須指向目前選取的 slide。');const slide=spec.slides[currentIndex()];if(p.region==='content.title'||p.region==='content.subtitle')return executeOperation({operation:'edit-text',target:{slideId:p.slideId,elementId:roleElementIds[p.region.slice(8)]},value:String(p.value)});const m=/^content\.keyPoints\.(\d+)$/.exec(p.region);if(m){const i=Number(m[1]);if(i>=slide.content.keyPoints.length)throw new Error('keyPoint index 超出範圍。');return executeOperation({operation:'edit-text',target:{slideId:p.slideId,elementId:pointElementId(slide.content.keyPointIds[i])},value:String(p.value)})}syncText();const c=/^content\.components\.([a-z0-9][a-z0-9._-]{0,79})$/i.exec(p.region);if(c&&p.value&&typeof p.value==='object'){const i=slide.content.components.findIndex(x=>x.id===c[1]);if(i<0)throw new Error('找不到 component。');const before=clone(slide.content.components[i]);slide.content.components[i]={...slide.content.components[i],...clone(p.value),id:c[1],type:slide.content.components[i].type};try{spec=clean()}catch(error){slide.content.components[i]=before;throw error}const component=spec.slides[currentIndex()].content.components.find(x=>x.id===c[1]);if(!component)throw new Error('元件 patch 未通過 sanitizer。');replaceComponent(component);return spec}throw new Error('patch region 不在 allowlist。')};
...
window.PPTSKILLEditor={getDeckSpec:()=>clone(clean()),operationDescriptors:clone(operationDescriptors),executeOperation:o=>clone(executeOperation(o)),applyLocalPatch:p=>{const out=applyPatch(p);status('已套用本機 AI patch');return clone(out)},replaceImageFile,prepareExport,exportHtml,getSizeReport:()=>prepareExport().report,download};
