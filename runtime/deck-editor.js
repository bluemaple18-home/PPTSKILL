...
const mutationBlocked=()=>mutationOwner||rollbackFailed;
const assertMutationAvailable=()=>{if(rollbackFailed)throw new Error('rollback failed：請重新掛載編輯器。');if(mutationOwner)throw new Error('同步交易進行中，拒絕重入。')};
const previewEvent=handler=>event=>{if(mutationBlocked())return false;mutationOwner=true;try{const result=handler(event);refreshHistoryControls(true);return result}finally{mutationOwner=false}};
const mutationEvent=handler=>event=>{if(mutationBlocked())return false;return runMutation(()=>handler(event))};
const history=createEditorHistory();
...
const setSelectedImageFit=fit=>{if(cropDialog?.isOpen()||pendingTextInsertion||imagePickerOpen||!['contain','cover'].includes(fit))return;const target=selectedImageTarget();if(!target)return;layout.cancel('image-fit');const component=selectedImageComponent(target);if((component.fit||'contain')===fit)return;try{executeOperationInternal({operation:'replace-asset',target,value:{dataUri:component.dataUri,fit}})}catch(error){status(error.message)}};
...
try{if(pending.mode==='edit'){executeOperationInternal({operation:'edit-text',target:{slideId:pending.slideId,elementId:pending.elementId},value:textInsertionInput.value});closeInsertText();status('文字已儲存');return}const used=new Set(slide.content.components.map(c=>c.id));let n=1;while(used.has('inserted-text-'+n))n++;
executeOperationInternal({operation:'insert-element',target:{slideId:pending.slideId},value:{component:{id:'inserted-text-'+n,type:'text',text:textInsertionInput.value},geometry:{x:560,y:288,width:480,height:320}}});closeInsertText();status('文字已插入')}
...
const applyStyle=operation=>{try{if(!editMode||!typographyTarget||typographyTarget.slideId!==currentId)throw new Error('請先選取目前頁面的標題或副標題。');if(composingText)throw new Error('IME 組字尚未完成，暫不可複製或貼上字級。');const request={operation,target:{...typographyTarget},value:{}},slide=spec.slides.find(s=>s.id===currentId);roleTypography.validateStyleTarget(slide,request);if(operation==='copy-style')roleTypography.copyStyle(slide,request);else if(!styleClipboard)throw new Error('請先複製明示字級。');syncText();executeOperationInternal(request);status(operation==='copy-style'?'已複製字級':'已貼上字級')}catch(error){status(error.message)}};
const applyTypography=reset=>{try{if(!editMode||!typographyTarget||typographyTarget.slideId!==currentId)throw new Error('請先選取目前頁面的標題或副標題。');if(composingText)throw new Error('IME 組字尚未完成，暫不可修改字級。');const input=q('[data-typography-size]'),raw=input?.value?.trim(),value=reset?null:raw?Number(raw):NaN,request={operation:'set-typography',target:{...typographyTarget},value:{fontSize:value}};roleTypography.validateRequest(request);syncText();executeOperationInternal(request);if(reset&&input)input.value='';status(reset?'已還原預設字級':'字級已更新')}catch(error){status(error.message)}};
...
let rollbackFault=null;
if(node&&parent&&(node.parentNode!==parent||node.nextSibling!==next)){
let proto=Object.getPrototypeOf(parent);while(proto&&!Object.hasOwn(proto,'insertBefore'))proto=Object.getPrototypeOf(proto);
try{if(!proto?.insertBefore)throw new Error('reorder DOM rollback primitive 不存在。');proto.insertBefore.call(parent,node,next)}catch(fault){rollbackFault=fault}
}
...
if(node&&parent&&(node.parentNode!==parent||node.nextSibling!==next))throw new Error('reorder DOM rollback 失敗：'+(rollbackFault?.message||error.message),{cause:error});
throw error
}};
...
const runMutation=(work,projection=[],afterRollback)=>{
assertMutationAvailable();mutationOwner=true;
let saved;
try{
...
if(afterRollback)try{afterRollback(error);saved.status=q('[data-editor-status]')?.textContent}catch(e){rollbackFailed=true;throw new Error('rollback failed：'+e.message,{cause:error})}
...
const executeOperationInternal=(request,afterCommit=()=>{})=>{const o=validateOperationRequest(request),before=clone(spec);const result=executeOperationCore(request);if(!sameCanonicalValue(before,spec))history.record(before,clone(spec),operationDescriptors[o.operation]?.undoable===true);afterCommit();return result};
...
document.addEventListener('click',mutationEvent(e=>{const slide=e.target.closest?.('.slide');if(slide)select(slide.dataset.slideId);const action=e.target.closest?.('[data-action]')?.dataset.action;if(!action)return;if(action==='crop-selected-image')cropDialog.open();if(action==='confirm-crop')cropDialog.submit(false);if(action==='reset-image-crop')cropDialog.submit(true);if(action==='cancel-crop')cropDialog.close();if(action==='insert-text')openInsertText();if(action==='edit-selected-text')openEditText();if(action==='submit-insert-text')submitInsertText();if(action==='cancel-insert-text'&&!textInsertionComposing)closeInsertText();if(action==='insert-image')openInsertImagePicker();if(action==='replace-selected-image')openSelectedImagePicker();if(action==='set-selected-image-fit')setSelectedImageFit(e.target.closest('[data-image-fit]')?.dataset.imageFit);if(action==='copy-style'||action==='paste-style')applyStyle(action);if(action==='apply-typography'||action==='reset-typography')applyTypography(action==='reset-typography');if(action==='edit')setEdit(!editMode);if(action==='edit-component')openComponentEditor();if(action==='apply-component')applyComponentEditor();if(action==='cancel-component')q('[data-component-dialog]').close();if(action==='move-up')move(-1);if(action==='move-down')move(1);if(action==='duplicate')duplicate();if(action==='delete')remove();if(action==='undo'||action==='redo'){refreshHistoryControls(true);if(!q('[data-action="'+action+'"]')?.disabled)try{replayHistory(action)}catch(error){status(error.message)}}if(action==='save')download()}));
const textEventRoot=document.body?.addEventListener?document.body:document;
textEventRoot.addEventListener('dblclick',mutationEvent(e=>{
...
textEventRoot.addEventListener('focusin',mutationEvent(e=>{const target=resolveDirectTextTarget(e.target);if(!editMode)return;if(target){select(target.slideId);setTypographyTarget(target)}else if(!e.target.closest?.('.pptskill-editor,[data-pptskill-typography-toolbar]'))clearTypographyTarget()}));
textEventRoot.addEventListener('compositionstart',mutationEvent(e=>{const target=resolveDirectTextTarget(e.target);if(editMode&&target)composingText=target.element}));
textEventRoot.addEventListener('compositionend',mutationEvent(e=>{const target=resolveDirectTextTarget(e.target);if(!target||composingText!==target.element)return;composingText=null;commitTextElement(target.element);status('文字已更新')}));
textEventRoot.addEventListener('focusout',mutationEvent(e=>{const target=resolveDirectTextTarget(e.target);if(!editMode||!target||composingText===target.element)return;commitTextElement(target.element)}));
textEventRoot.addEventListener('keydown',mutationEvent(e=>{if(e.key!=='Escape'||e.isComposing)return;const target=resolveDirectTextTarget(e.target);if(!editMode||!target||composingText===target.element)return;restoreTextElement(target.element);e.preventDefault();e.stopImmediatePropagation?.();status('已取消文字變更')}));
const replaceImageFile=async(file,explicitTarget)=>{
assertMutationAvailable();let started=false,target,pending;
...
try{
if(!file)return null;
runMutation(()=>{if(!window.PPTSKILLAssets)throw new Error('Asset optimizer 未載入。');
...
requireImage();pendingAssetOperations++;status('正在最佳化圖片…');pending=window.PPTSKILLAssets.optimizeFile(file)});started=true;
const result=await pending;
const output=runMutation(()=>{requireImage();const message=result.warnings.length?'圖片已替換（保留大型 GIF）':result.optimized?'圖片已最佳化並替換':'圖片已替換';executeOperationInternal({operation:'replace-asset',target,value:{dataUri:result.dataUri}});const changed=selectedImageComponent(target);status(changed&&imageCrop.state(changed)==='pending'?'裁切待重新確認：目前完整顯示原圖。':message);pendingAssetOperations--;return result});started=false;return output;
}finally{if(started)runMutation(()=>{pendingAssetOperations--})}};
const insertImageFile=async(file,options)=>{
assertMutationAvailable();let started=false,request,pending;
try{
if(!file)return null;
runMutation(()=>{if(typeof window.PPTSKILLAssets?.optimizeFile!=='function')throw new Error('Asset optimizer 未載入。');request=imageInsertion.snapshotFileOptions(options);imageInsertion.preflight(spec.slides.find(s=>s.id===request.target.slideId),request);pendingAssetOperations++;pending=window.PPTSKILLAssets.optimizeFile(file)});started=true;
const result=await pending;
const output=runMutation(()=>{if(!result||typeof result!=='object'||typeof result.dataUri!=='string'||!Array.isArray(result.warnings)||result.warnings.some(w=>typeof w!=='string')||typeof result.optimized!=='boolean')throw new Error('Asset optimizer result 無效。');const message=result.warnings.length?'圖片已插入（保留大型 GIF）':result.optimized?'圖片已最佳化並插入':'圖片已插入';request.value.component.dataUri=result.dataUri;executeOperationInternal(request);status(message);pendingAssetOperations--;return result});started=false;return output;
}finally{if(started)runMutation(()=>{pendingAssetOperations--})}};
...
document.addEventListener('keydown',e=>{if(mutationBlocked())return;if(e.defaultPrevented||e.isComposing||e.altKey||!(e.ctrlKey||e.metaKey)||e.key?.toLowerCase()!=='z')return;const node=e.target?.closest?e.target:e.target?.parentElement,active=document.activeElement;if(pasteInputOwned(node)||pasteInputOwned(active)||node?.closest?.('dialog')||active?.closest?.('dialog'))return;const direction=e.shiftKey?'redo':'undo';try{if(runMutation(()=>replayHistory(direction))){e.preventDefault();e.stopImmediatePropagation?.()}}catch(error){status(error.message)}},true);
...
qa('.slide').forEach(slide=>slide.addEventListener('focusin',mutationEvent(()=>select(slide.dataset.slideId))));select(currentId);document.body.dataset.editorMode='play';
