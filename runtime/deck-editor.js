...
catch(error){spec=before;revision=revisionBefore;for(const {node,style,geometry} of nodes)for(const [key,value] of [['style',style],['data-pptskill-geometry',geometry]]){if(value===null)node.removeAttribute(key);else if(node.getAttribute(key)!==value)node.setAttribute(key,value)}throw error}
finally{groupCommitInProgress=false}
};
const executeOperation=request=>{const o=validateOperationRequest(request);if(groupLock.names.includes(o.operation))return commitGroup(o);const groupSlide=spec.slides.find(s=>s.id===o.target.slideId);if(groupSlide)groupLock.assertMutable(groupSlide,o.operation,o.target.elementIds||[o.target.elementId]);if(o.operation==='delete-element')return deleteComponent(o);if(o.operation==='insert-element'){
