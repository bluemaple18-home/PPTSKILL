...
export function createDeckEditor(input) {
  ...
  const executeOperation = (request) => {
    ...
    if (!editTextAllowedTargetRoles.includes(role)) throw new Error(`operation 不支援 target role：${role}`);
...
const supportedChartTypes=${JSON.stringify(chartCapability.supportedChartTypes)},chartValueDomain=${JSON.stringify(chartCapability.valueDomain)},operationDescriptors=${JSON.stringify(OPERATION_DESCRIPTORS)},editTextAllowedTargetRoles=Object.freeze(['title','subtitle','keyPoint']);
