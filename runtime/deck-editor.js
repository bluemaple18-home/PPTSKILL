...
export function createDeckEditor(input) {
  ...
  if (rawMotionErrors.length) throw new Error(rawMotionErrors.join(' '));
...
const updateMetricPoint=(slide,index,point)=>{const article=q('.metric-cards article[data-metric-index="'+index+'"]',currentNode());if(!article)return;const parts=String(point).split('｜'),value=parts.shift(),label=parts.join('｜'),target=slide.composition.motion?.targets.find(t=>t.ref==='content.keyPoints.'+index),valueNode=q('.metric-value',article),labelNode=q('p',article);if(target){const parsed=parseMotionMetric(point),flow=q('[data-pptskill-odometer]',article);if(!parsed||!flow)throw new Error('motion metric DOM 與 canonical content 失配。');const spans=qa(':scope > span',valueNode);if(spans[0])spans[0].textContent=parsed.numberPrefix;if(spans[1])spans[1].textContent=parsed.numberSuffix;flow.dataset.to=String(parsed.finalValue);flow.dataset.finalDisplay=parsed.numericDisplay;flow.dataset.useGrouping=String(parsed.format.useGrouping);flow.dataset.fractionDigits=String(parsed.format.maximumFractionDigits);flow.animated=false;if(typeof flow.update==='function')flow.update(parsed.finalValue,parsed.format);else flow.textContent=parsed.numericDisplay}else if(valueNode)valueNode.textContent=value;if(labelNode)labelNode.textContent=label};
const applyPatch=p=>{syncText();if(!p||p.slideId!==currentId)throw new Error('patch 必須指向目前選取的 slide。');const slide=spec.slides[currentIndex()];if(p.region==='content.title'||p.region==='content.subtitle'){const field=p.region.slice(8),before=clone(spec),el=qa('[data-edit-target]',currentNode()).find(x=>x.dataset.editTarget==='slides.'+currentId+'.content.'+field),beforeText=el?.textContent;try{slide.content[field]=String(p.value);spec=clean();if(el)el.textContent=spec.slides[currentIndex()].content[field];return spec}catch(error){spec=before;if(el)el.textContent=beforeText;throw error}}const m=/^content\.keyPoints\.(\d+)$/.exec(p.region);if(m){const i=Number(m[1]);if(i>=slide.content.keyPoints.length)throw new Error('keyPoint index 超出範圍。');const before=clone(spec),article=q('.metric-cards article[data-metric-index="'+i+'"]',currentNode()),beforeHtml=article?.innerHTML;try{slide.content.keyPoints[i]=String(p.value);const cleaned=clean();updateMetricPoint(cleaned.slides[currentIndex()],i,cleaned.slides[currentIndex()].content.keyPoints[i]);spec=cleaned;return spec}catch(error){spec=before;if(article&&beforeHtml!==undefined)article.innerHTML=beforeHtml;throw error}}const c=/^content\.components\.([a-z0-9][a-z0-9._-]{0,79})$/i.exec(p.region);if(c&&p.value&&typeof p.value==='object'){const i=slide.content.components.findIndex(x=>x.id===c[1]);if(i<0)throw new Error('找不到 component。');const before=clone(slide.content.components[i]);slide.content.components[i]={...slide.content.components[i],...clone(p.value),id:c[1],type:slide.content.components[i].type};try{spec=clean()}catch(error){slide.content.components[i]=before;throw error}const component=spec.slides[currentIndex()].content.components.find(x=>x.id===c[1]);if(!component)throw new Error('元件 patch 未通過 sanitizer。');replaceComponent(component);return spec}throw new Error('patch region 不在 allowlist。')};
