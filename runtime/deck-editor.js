...
window.PPTSKILLEditor={getDeckSpec:()=>clone(clean()),operationDescriptors:operationDescriptors,executeOperation:o=>clone(executeOperation(o)),applyLocalPatch:p=>{const out=applyPatch(p);status('已套用本機 AI patch');return clone(out)},replaceImageFile,prepareExport,exportHtml,getSizeReport:()=>prepareExport().report,download};
