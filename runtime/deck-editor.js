...
const refreshSelectedImage=reason=>{if(reason!=='blur'&&reason!=='refresh')pendingImageTarget=null;const button=q('[data-action="replace-selected-image"]'),target=selectedImageTarget();if(button){button.hidden=!target;button.disabled=!target||imagePickerOpen}};
