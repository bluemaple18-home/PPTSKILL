...
let pendingImageTarget=null,imagePickerOpen=false;
...
const refreshSelectedImage=reason=>{if(reason!=='blur'&&reason!=='refresh')pendingImageTarget=null;const button=q('[data-action="replace-selected-image"]'),target=selectedImageTarget(),group=q('[data-selected-image-fit]'),fit=target?(selectedImageComponent(target).fit||'contain'):null;if(button){button.hidden=!target;button.disabled=!target||imagePickerOpen}if(group)group.hidden=!target;qa('[data-image-fit]').forEach(control=>{control.disabled=!target||imagePickerOpen;control.setAttribute('aria-pressed',String(control.dataset.imageFit===fit))})};
...
selectedImageInput?.addEventListener('change',async e=>{const target=pendingImageTarget,file=e.target.files?.[0];pendingImageTarget=null;imagePickerOpen=false;e.target.value='';refreshSelectedImage('refresh');if(!file)return;if(!target){status('圖片選取已失效，請重新選取圖片。');return}try{await replaceImageFile(file,target)}catch(error){status(error.message)}});
