...
q('#pptskill-image-input')?.addEventListener('change',e=>{const file=e.target.files?.[0];if(!file)return;if(!['image/png','image/jpeg','image/webp','image/gif','image/svg+xml'].includes(file.type)){status('不支援的圖片格式');return}const slide=spec.slides[currentIndex()],component=slide.content.components.find(c=>c.type==='image');if(!component){status('此頁沒有可替換圖片');return}const reader=new FileReader();reader.onload=()=>{component.dataUri=reader.result;const img=q('.slide[data-slide-id="'+CSS.escape(currentId)+'"] [data-edit-target="slides.'+CSS.escape(currentId)+'.content.components.'+CSS.escape(component.id)+'"] img');if(img)img.src=reader.result;status('圖片已替換')};reader.readAsDataURL(file)});
...
window.PPTSKILLEditor={getDeckSpec:()=>clone(clean()),applyLocalPatch:p=>{const out=applyPatch(p);status('已套用本機 AI patch');return clone(out)},exportHtml};
