...
const buildInsertTextToolbarMarkup = () => `<span data-pptskill-insert-text-toolbar><button type="button" data-action="insert-text" hidden disabled>插入文字</button></span>`;
...
const select=id=>{if(currentId!==id){closeInsertText();layout?.clearSelection();clearTypographyTarget()}currentId=id;qa('.slide').forEach(s=>s.dataset.editorSelected=String(s.dataset.slideId===id));refreshInsertImage();};
...
const refreshInsertText=()=>{if(!layout?.getState().enabled)textInsertionDialog?.removeAttribute('data-pptskill-editor-chrome');const button=q('[data-action="insert-text"]');if(button){button.hidden=!insertionSlide();button.disabled=button.hidden||insertionBusy||Boolean(imagePickerOpen)||Boolean(pendingTextInsertion)}};
...
layout.cancel('insert-text');pendingTextInsertion={slideId:slide.id,root,focus:q('[data-action="insert-text"]')};textInsertionInput.value='';textInsertionStatus.textContent='';refreshSelectedImage('refresh');
...
try{const used=new Set(slide.content.components.map(c=>c.id));let n=1;while(used.has('inserted-text-'+n))n++;
