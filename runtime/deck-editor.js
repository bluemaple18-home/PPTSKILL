...
let deletionInProgress=false,pendingAssetOperations=0,cropCommitInProgress=false;
...
const executeOperation=request=>{const o=validateOperationRequest(request);if(o.operation==='delete-element')return deleteComponent(o);if(o.operation==='insert-element'){
...
if(deletionInProgress)throw new Error('delete-element 進行中，拒絕同步重入。');pendingAssetOperations++;try{
...
executeOperation({operation:'replace-asset',target,value:{dataUri:result.dataUri}});const changed=selectedImageComponent(target);status(changed&&imageCrop.state(changed)==='pending'?'裁切待重新確認：目前完整顯示原圖。':message);return result}finally{pendingAssetOperations--}};
...
request.value.component.dataUri=result.dataUri;executeOperation(request);status(message);return result}finally{pendingAssetOperations--}};
