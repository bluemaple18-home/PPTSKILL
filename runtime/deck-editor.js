...
const executeOperation=request=>{const o=validateOperationRequest(request);if(o.operation==='replace-asset'){
...
if(!sameCanonicalValue(before,spec))assetReplacement.project(img,component);markChanged(before,spec);return spec}catch(error){spec=before;for(const [key,value] of attributes){if(value===null)img.removeAttribute(key);else img.setAttribute(key,value)}throw error}}
...
const replaceImageFile=async file=>{if(!file)return null;if(!window.PPTSKILLAssets)throw new Error('Asset optimizer 未載入。');const slide=spec.slides[currentIndex()],index=slide.content.components.findIndex(c=>c.type==='image');if(index<0)throw new Error('此頁沒有可替換圖片。');const target={slideId:slide.id,elementId:resolveSlideElementIdentities(slide).components[index]};status('正在最佳化圖片…');const result=await window.PPTSKILLAssets.optimizeFile(file);const live=spec.slides.find(s=>s.id===target.slideId);if(!live||!live.content.components.some((c,i)=>c.type==='image'&&resolveSlideElementIdentities(live).components[i]===target.elementId))throw new Error('圖片元件已移除。');executeOperation({operation:'replace-asset',target,value:{dataUri:result.dataUri}});status(result.warnings.length?'圖片已替換（保留大型 GIF）':result.optimized?'圖片已最佳化並替換':'圖片已替換');return result};
