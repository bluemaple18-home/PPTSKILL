...
.pptskill-editor [data-pptskill-typography-toolbar]{align-items:center;gap:4px}.pptskill-editor [data-typography-size]{width:5em;background:#151515;color:inherit;border:1px solid #ffffff50;border-radius:4px;font:inherit;padding:4px}.pptskill-editor [data-typography-size]:focus-visible{outline:2px solid #9ed7ff;outline-offset:2px}
...
const tag=q('#deck-spec');if(!tag)return;let spec=JSON.parse(tag.textContent);const safeStyle=clone(spec.style);let currentId=spec.slides[0]?.id||'';let editMode=false;let editingComponentId='',composingText=null,typographyTarget=null;
...
const select=id=>{if(currentId!==id){layout?.clearSelection();clearTypographyTarget()}currentId=id;qa('.slide').forEach(s=>s.dataset.editorSelected=String(s.dataset.slideId===id));};
...
const setEdit=on=>{if(on&&layout?.getState().enabled)layout.setMode(false);let cancelledComposition=false;if(!on&&editMode){if(composingText){restoreTextElement(composingText);composingText=null;cancelledComposition=true}syncText()}editMode=on;if(!on)clearTypographyTarget();document.body.dataset.editorMode=on?'edit':'play';qa('[data-edit-kind="text"]').forEach(el=>el.contentEditable='false');directTextElements().forEach(el=>el.contentEditable=String(on));q('[data-action="edit"]').textContent=on?'完成編輯':'編輯文字';status(on?'正在編輯':cancelledComposition?'未完成的組字已取消':'變更已保留')};
...
textEventRoot.addEventListener('focusin',e=>{const target=resolveDirectTextTarget(e.target);if(!editMode)return;if(target){select(target.slideId);setTypographyTarget(target)}else if(!e.target.closest?.('[data-pptskill-typography-toolbar]'))clearTypographyTarget()});
