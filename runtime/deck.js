...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    #presenter { position: fixed; right: 1rem; top: 1rem; max-width: 24rem; padding: 1rem; border-radius: .6rem; background: #fff; color: #15213a; box-shadow: 0 8px 28px #0008; }
    #presenter p { margin: .4rem 0; }
...
<body data-mode="play">
  ...
  <aside id="presenter" hidden aria-live="polite">
    <strong>Presenter</strong>
    <p id="notes"></p><p id="next-hint"></p><p id="source"></p><p id="progress"></p>
  </aside>
  <nav id="controls" aria-label="播放控制">
    <button data-action="previous">上一頁</button><button data-action="next">下一頁</button>
    <button data-action="presenter">講者模式</button><button data-action="edit">編輯模式</button>
  </nav>
  ...
  <script>
    (() => {
      ...
      const render = () => {
        ...
        document.querySelector('#notes').textContent = '講稿：' + slide.notes;
        document.querySelector('#next-hint').textContent = '下一頁提示：' + slide.nextHint;
        document.querySelector('#source').textContent = '來源：' + slide.source;
        document.querySelector('#progress').textContent = '進度：' + (current + 1) + ' / ' + slides.length;
        presenter.hidden = !presenterVisible;
      };
      const saveEdits = () => {
        view.querySelectorAll('.slide').forEach((element, index) => {
      ...
      document.addEventListener('click', (event) => {
        ...
        if (action === 'presenter') presenterVisible = !presenterVisible;
