...
const runs = [];
const snap = process.argv.includes('--snap-regression');
for (const mib of [1, 6, 12]) {
  const h = mountedEditor(fixture(mib)); h.ready();
  if (snap) h.action('snap-layout');
  h.begin();
  // payload 是 mounted vendor double 的 canonical 候選；量化由真 browser coverage 驗證。
  const update = i => snap ? h.vendor.handlers.drag({ inputEvent: { clientX: i + 1, clientY: i + 1 }, left: 816, top: 296 }) : h.update(i % 20 + 1, i % 20 + 1);
  for (let i = 0; i < 30; i++) update(i);
  h.resetCounts(); const times = [];
  for (let i = 0; i < 101; i++) {
    const start = performance.now(); update(i); times.push(performance.now() - start);
  }
  assert.deepEqual(h.counts, { payloadReads: 0, serializations: 0, wholeSpecSerializations: 0 });
  const counts = { ...h.counts };
  h.finish(); assert.deepEqual(geometry(h.getSpec()), { ...box, x: snap ? 816 : 801, y: snap ? 296 : 281 });
  times.sort((a, b) => a - b);
...
const receipt = { status: 'pass', snap, scope: 'generated browser runtime mounted update+preview；DOM/vendor double，非真 browser/frame timing', gate: 'payloadReads=serializations=wholeSpecSerializations=0；release committed once', node: process.version, sourceSha256, runs };
