...
async function buildEqualGapFixture(outputDir) {
  ...
  assert.ok(portable, 'S11 browser fixture 缺 portable slide');
  portable.content.components.push(
    { id: 's11-two', type: 'text', text: '第二個等間距元件' },
  ...
  portable.composition.geometryOverrides = {
    ...
    's11-two': { x: 620, y: 560, width: 240, height: 160 },
  ...
  const path = resolve(outputDir, 's11-source.html');
  await writeFile(path, rendered.html);
  return { path, spec: extractDeckSpec(rendered.html) };
}

export async function runEqualGapBrowserCases({ cdp, evaluate, navigate, outputDir, run, click, position, settle, assertExport }) {
  const fixture = await buildEqualGapFixture(outputDir);
  ...
  const clickWithShift = async css => {
    ...
    for (const [type, buttons] of [['mouseMoved', 0], ['mousePressed', 1], ['mouseReleased', 0]]) {
      await cdp.send('Input.dispatchMouseEvent', { type, x: point.x, y: point.y,
        button: type === 'mouseMoved' ? 'none' : 'left', buttons, clickCount: type === 'mouseMoved' ? 0 : 1, modifiers: 8 });
    }
    await settle();
  };
  ...
  assert.equal(await hidden('distribute-horizontal-gaps'), true, '2 選取不得顯示 equal-gap');
  await clickWithShift(`${componentSelector}[data-pptskill-element-id="${ids[2]}"]`);
  assert.deepEqual(await selected(), ids);
  assert.equal(await hidden('distribute-horizontal-gaps'), false);
  assert.equal(await hidden('distribute-vertical-gaps'), false);
  run.checks.push({ s11: 'equal-gap-context', selected: ids });

  await click('[data-action="distribute-horizontal-gaps"]');
  assert.deepEqual(await geometry('portable-quote'), { x: 180, y: 260, width: 360, height: 220 });
  assert.deepEqual(await geometry('s11-two'), { x: 740, y: 560, width: 240, height: 160 });
  assert.deepEqual(await geometry('s11-three'), { x: 1180, y: 100, width: 160, height: 120 });
  assert.deepEqual(await selected(), ids);
  run.checks.push({ s11: 'horizontal-gaps', selected: ids });

  await click('[data-action="distribute-vertical-gaps"]');
  assert.deepEqual(await geometry('portable-quote'), { x: 180, y: 280, width: 360, height: 220 });
  assert.deepEqual(await geometry('s11-two'), { x: 740, y: 560, width: 240, height: 160 });
  assert.deepEqual(await geometry('s11-three'), { x: 1180, y: 100, width: 160, height: 120 });
  assert.deepEqual(await selected(), ids);
  const committed = await spec();
  const exported = await assertExport('s11-equal-gap-export', committed);
  ...
  run.checks.push({ s11: 'vertical-gap-export-reopen', selectionRetainedBeforeExport: true, reopenedSelection: [] });
  run.artifacts.push({ label: 's11-source', path: fixture.path });
}
