...
if (process.argv.includes('--asset-replacement-regression') || process.argv.includes('--targeted-image-file-regression') || process.argv.includes('--selected-image-regression') || process.argv.includes('--image-fit-regression') || process.argv.includes('--insert-image-regression')) addAssetReplacementFixture(input);
...
try {
  for (const [width, height] of [[1280, 720], [1600, 900]]) {
    ...
    try {
      ...
      if (process.argv.includes('--insert-image-regression')) await runInsertImageBrowserCases({ cdp, evaluate, navigate, sourcePath, outputDir, width, run, click, position, mouse, settle, assertExport });
