...
- 在架構確認後產生四個同內容候選：一個 Company Style 與三個依本次簡報動態編譯的 AI Visual Routes；既有四 Theme 僅作參考與 fallback。
- 將核心流程與 runtime 包裝分離，讓同一套規格、模板及驗收規則能跨 AI 重用。
- 以 ZIP 提供一份共用核心與 Codex、Claude Code、Gemini CLI 薄入口；使用者使用自己的 AI 帳號與 quota。
- 在單檔 HTML 內嵌經 sanitizer 處理的 versioned DeckSpec，使另一個相容 AI 能只靠該檔修改與重建。
...
- Presenter Mode、中央儲存／發布、帳號／ACL／DRM、自由 x/y 拖拉、通用 PPTX importer。
...
- 先以受眾視角定義敘事頁序；outline 每頁只鎖定大標、小標與 3～5 個重點，再進入完整內容生成。
- 每頁以 slide ID 定位；DeckSpec 分離內容、StyleSpec 與 CompositionSpec，讓換排版不改內容、單頁修改不重生整份。
...
- **D-009 / Superseded by D-023**：既有四套 Theme 降為 reference／fallback，不再是公版固定四選一。
- **D-010 / Superseded by D-023**：風格選擇仍在 outline 確認後，但候選改為 Company Style＋三個本次動態 AI Visual Routes。
...
- **D-020 / Superseded by D-024**：Presenter Mode 與講者稿不屬 MVP；既有 PS-001 實作只保留為歷史證據，後續 runtime 與測試須移除該表面。
...
- **D-023 / Resolved**：四個風格候選固定為一個 Company Style slot 與三個依本次內容產生的 AI Visual Routes；共用相同已核准封面內容，且必須是真實 HTML/CSS renderer output。
- **D-024 / Resolved**：MVP 保留播放與直接編輯，但移除 Presenter Mode；可修改文字與支援元件、替換圖片、重排、複製、刪除、指定區域 AI patch 與另存新 HTML。
- **D-025 / Resolved**：MVP outline 為 1～15 頁；每頁包含 `title`、`subtitle`、3～5 個 `key_points`。16 頁以上須停在 outline gate 由使用者合併、刪除或拆分。
- **D-026 / Resolved**：單檔 HTML 內嵌 versioned DeckSpec、StyleSpec 與 CompositionSpec；export sanitizer 只允許最終內容與重建所需資料，排除素材全文、訪談、profile、本機路徑、prompt、推理及退稿。
- **D-027 / Resolved**：完整產製必須 bounded；禁止一次生成完整 15 頁、重送所有先前頁面內容、四倍生成內容、單頁修改重生全份或換排版改內容。
- **D-028 / Resolved**：非預期文字／圖片碰撞、overflow 與 off-canvas 為 release blocker；修復順序為較安全 composition → 明確縮短內容 → 拆頁，不得無限縮字。
- **D-029 / Resolved**：員工取得 ZIP 而非 Git repo；ZIP 內是一份共用核心與三個 CLI 薄入口，更新不得覆寫位於安裝目錄之外的選配個人 profile。
...
- **US-002 / 簡報者**：身為簡報者，我要在不離開單檔 HTML 的情況下播放、直接修稿並另存新檔，以便安全地展示與交付成品。
...
- **FR-003 / Deterministic validation**：系統 SHALL 執行 schema、sanitizer、單檔、資產、碰撞、overflow、off-canvas、檔案大小與 browser geometry 檢查，並將結果寫入 manifest。
...
- **FR-005 / Portable editor safety**：系統 SHALL 保留直接 HTML 編輯與指定區域 AI patch，並以 sanitizer 確保另存 HTML 不含素材全文、訪談、profile、本機路徑、prompt、推理或退稿。
...
- **SC-003**：MVP runtime 的播放、前後導航、直接編輯、圖片替換、重排、複製、刪頁、指定區域 AI patch 與另存 smoke 均 pass；Presenter 不得出現在 MVP surface。
...
| `outline.json` | brief_valid → outline_confirmed | `brief.md` | 1～15 個唯一 slide ID；每頁大標、小標與 3～5 個重點 | 目前 runtime | 產製者確認頁序與內容 |
| `theme-preview-manifest.json` | outline_confirmed → theme_selected | `outline.json`、Company Style Pack、動態 routes | 同內容的四張真實 HTML/CSS 封面、route diversity 與人工選款 receipt | 目前 runtime | 使用者選定 StyleSpec |
| `deck.html` | theme_selected → full_built | 已選 StyleSpec、outline、bounded generation units | 內嵌 sanitized versioned DeckSpec 的離線單檔 deck、播放與直接編輯能力 | 目前 runtime | deterministic gate 與人工檢視 |
| `render-evidence.json` | full_built → evidence_valid | `deck.html`、capability probe | schema、sanitizer、資產、碰撞／overflow／off-canvas、兩尺寸 browser evidence 與最後成功 artifact | 目前 runtime | deterministic gate |
...
| `outline_confirmed` | `theme_selected` | Company Style＋三個 dynamic routes 已真實渲染，使用者選定 StyleSpec | 留在 outline_confirmed，保留 outline 與預覽 |
| `theme_selected` | `full_built` | bounded generation 完成且 `deck.html` 內嵌 sanitized DeckSpec | 留在 theme_selected，保留選定 StyleSpec 與最後成功 generation unit |
...
| outline 完整性 | 每頁 title、subtitle、3～5 key points 與唯一 slide ID | 缺欄、重複 ID、超過 15 頁即 fail |
| 單檔／無外鏈 | `deck.html` 的資產與連結掃描 | 外部依賴或不可離線資產即 fail |
| 資產載入 | fresh state 載入結果 | 任一必要資產失敗即 fail |
| browser geometry | 兩尺寸 screenshot、碰撞、overflow 與 off-canvas 記錄 | 缺證據或任何未修復幾何錯誤即 fail |
| 編輯／另存 smoke | 播放、導航、直接編輯、圖片替換、重排、複製、刪頁、指定區域 AI patch 與另存結果 | 任一必要操作不可用或 sanitizer 洩漏即 fail |
...
1. **Company Style**：由 owner 後續提供的單一公司 PPTX 一次轉為 reviewed Style Pack；MVP 不做通用 importer。
2. **AI Visual Route A／B／C**：依本次內容從已驗證的前端設計材料與 primitives 編譯，三者須在 composition、typography、density、geometry、palette roles、asset treatment 或 motion 上形成實質差異。

既有 Executive Clear、Product Blueprint、Sales Momentum、Brand Story 只保留為 reference／fallback，不再作固定四選一契約。Style 在整份 deck 維持一致；CompositionSpec 可逐頁變化。
...
2. 依已確認的真實標題與副標產出四張 16:9 真實 HTML/CSS 封面：Company Style＋三個 dynamic routes。
3. 四張圖的文字、資訊層級與素材條件保持一致；route diversity 不得只是換色。
4. 每張圖清楚標示 Style 名稱；可附一句適用情境，不要求使用者理解設計術語。
...
- **成品 runtime**：播放模式預設隱藏控制介面；使用者可進入直接編輯模式修改支援內容並以「另存新 HTML」保存結果，不直接覆寫來源檔。MVP 不提供 Presenter Mode。
...
既有**內部三頁功能測試樣本**與 evidence 保留為 PS-001 歷史證據；MVP repair 必須移除 Presenter surface，並補上 DeckSpec、安全 sanitizer、圖片替換、指定區域 AI patch 與另存後重開驗證。

此內部三頁功能測試樣本不是正式簡報或內容模板。MVP 正式簡報上限為 15 頁。
