...
1. layout mode正常slide显示「插入文字」按鈕；play/edit隱藏或disabled，沒有valid slide不得動作。Native modal dialog有label、textarea、取消／插入與就近status。bootstrap舊HTML補一次markup，remount不duplicate；既有圖片controls保持，雙viewport不clip。
