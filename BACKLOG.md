...
**S10 IN PROGRESS：** `tasks/edx-wp1-s10-distribute-selection.md`。沿 S9 atomic multi-selection seam 實作 `distribute-selection`；3+ component、頭尾固定、horizontal/vertical centers 等距，只讀 canonical geometryOverrides。Prior art 採 donor MIT distribution semantics，bundle dependency delta 0；equal-gap、group transform、history、WP2 均不在本卡。Branch `codex/edx-wp1-s10-distribute-selection`，base `cbe12a6`。
