...
**S9 IN PROGRESS：** `tasks/edx-wp1-s9-align-selection.md`。第一條 multi-selection mutation 限定為同頁 2+ component 的 atomic `align-selection`；六種 bbox 對齊，只接受既有 canonical geometry，不從 DOM 升格 truth。沿 Unified Operation Registry / geometryOverrides；不做 distribute、group transform、history、WP2。Branch `codex/edx-wp1-s9-align-selection`，base `f7537c4`。
