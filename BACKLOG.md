...
**S11 IN PROGRESS：** `tasks/edx-wp1-s11-equal-gap.md`。沿 S10 `distribute-selection` 補 `horizontal-gaps / vertical-gaps`；頭尾固定、不同尺寸元件等空白間距，negative gap 原子拒絕。donor 只有 center distribution，Moveable/Selecto 無直接 equal-gap mutation API；因此為 existing seam 上的 CUSTOM_DELTA，dependency delta 0。Branch `codex/edx-wp1-s11-equal-gap`，base `29c83fa`。
