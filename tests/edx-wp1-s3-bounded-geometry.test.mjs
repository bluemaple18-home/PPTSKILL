...
for (const [lane, makeEditor] of [['Node', createDeckEditor], ['browser API VM', browserEditor]]) {
  ...
  test(`${lane} descriptors 深度 immutable 且 allowlist 無法擴權`, () => {
    ...
    assert.deepEqual(Object.keys(d), ['replace-asset', 'set-typography', 'copy-style', 'paste-style', 'edit-text', 'move-element', 'resize-element', 'align-selection', 'distribute-selection']);
