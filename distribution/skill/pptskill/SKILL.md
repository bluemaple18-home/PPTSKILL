...
Before asking for sample approval, use `node <runtime-root>/core/runtime/workflow-cli.mjs qa-sample --request <qa-request.json>` only for bounded Slice 2 repair planning. Caller-authored checks are not approval authority. Preserve each returned `slideId + issue code`; `not_run`／`unknown` remain blocked. Execute at most the returned bounded repair action, never reset a problem's shared two-attempt budget, never overwrite the last-success artifact, and never treat sample PASS as full-deck PASS.

After explicit human approval, run `workflow-cli.mjs approve-sample --request <approval-request.json> --artifact <portable.html>`. The CLI runs its packaged Chrome producer and derives identity from the artifact's canonical DeckSpec; never put a hard gate, PASS, evidence object, or fingerprint in the request. Any DeckSpec/contract mismatch fails loud. Keep feedback bounded to `slide-local | deck-wide | profile-opt-in`, preserve freeze fingerprints, and repair/re-approve only invalidated sample slides. Apply deck-wide feedback only to unapproved slides. Never write profile preferences unless `remember: true`, never silently regenerate preserved samples, and always complete full-deck QA.

Before release, run `workflow-cli.mjs qa-full-deck --request <qa-request.json> --artifact <portable.html>`. The CLI obtains Layer-1 checks for every canonical slide from its packaged Chrome producer; caller-authored PASS, checks, coverage, or evidence are forbidden. Layer-2 advisories must use bounded codes and cite current trusted evidence. The first run may return `awaiting-owner` with the current identity; Layer-3 human confirmation must repeat that identity, and any artifact/DeckSpec/contract change requires confirmation again. Sample approval never replaces this full-deck gate.
