...
window.setup=(scale,grid,transform)=>{
 window.m?.destroy();document.querySelectorAll('[data-probe-overlay]').forEach(e=>e.remove());
 ...
 window.m=new PPTSKILLMoveable.default(overlay,{target,container:slide,rootContainer:document.body,snapContainer:slide,draggable:true,resizable:true,renderDirections:['se'],origin:false,snappable:grid>0,snapGridWidth:grid,snapGridHeight:grid,throttleDrag:0,throttleResize:0});
...
try{
 ...
 for(const transform of ['none','scale(0.8)','translateY(20px)'])for(const grid of [0,8])for(const kind of ['drag','resize']){
 await evaluate(`setup(${scale},${grid},${JSON.stringify(transform)})`);await settle();await evaluate("m.updateRect()");await settle();
 const position=await evaluate(`(()=>{const r=document.querySelector(${JSON.stringify(kind==='drag'?'#target':'.moveable-control.moveable-se')}).getBoundingClientRect();return{x:r.x+r.width/2,y:r.y+r.height/2}})()`);
 ...
 await mouse('mouseMoved',position.x+7*scale,position.y+7*scale,true);await mouse('mouseReleased',position.x+7*scale,position.y+7*scale,false);await settle();
 ...
 receipt.runs.push({width,scale,transform,grid,kind,requestedCanonicalDelta:7,observedCanonicalDelta:events.filter(e=>e.client).length>1?events.find(e=>e.type===kind).client.map((v,i)=>(v-events[0].client[i])/scale):null,events,cssBox:await evaluate('({left:target.offsetLeft,top:target.offsetTop,width:target.offsetWidth,height:target.offsetHeight})')});
 }}
 for(const key of ['console','pageErrors','networkFailures','httpErrors','remoteRequests'])assert.equal(receipt[key].length,0,key);
 assert.equal(receipt.runs.length,24);receipt.status='pass';
