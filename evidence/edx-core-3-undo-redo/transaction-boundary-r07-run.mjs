// repo root 執行：node evidence/edx-core-3-undo-redo/transaction-boundary-r07-run.mjs full rerun-01
...
const focused = ['tests/edx-core-2-group-lock.test.mjs', 'tests/edx-core-3-undo-redo.test.mjs', 'tests/edx-core-3-transaction-boundary.test.mjs', 'tests/edx-wp1-s4-perf.test.mjs', 'tests/edx-wp1-s4-interaction.test.mjs', 'tests/edx-wp1-s5-keyboard.test.mjs', 'tests/edx-wp1-s9-mounted-align.test.mjs', 'tests/edx-wp1-s10-mounted-distribute.test.mjs', 'tests/edx-wp1-s7-snap.test.mjs'];
const tests = mode === 'full' ? original.tests : focused;
const path = root + 'transaction-boundary-r07-' + label;
