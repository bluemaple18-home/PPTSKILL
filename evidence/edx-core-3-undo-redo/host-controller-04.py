...
out = repo / 'evidence/edx-core-3-undo-redo/host-acceptance-04'
node = '/opt/homebrew/bin/node'
frozen = json.loads((repo / 'evidence/edx-core-3-undo-redo/source-hashes.json').read_text())
def verify():
    result = {}
...
receipt['scope'] = 'Core 3 Undo/Redo product acceptance'
...
try:
    ...
    deadline = readiness_start+receipt['readinessDeadlineSeconds']
    portfile = None
    ...
    if not endpoint:
        ...
        receipt['readinessElapsedSeconds']=round(time.monotonic()-readiness_start, 3)
        raise RuntimeError('25 秒 readiness deadline 未完成')
    ...
    for key,args,limit in [
        ('historyBrowser',[node,'tools/edx-wp1-s4-browser-acceptance.mjs',str(out/'undo-redo'),'--undo-redo-regression'],900),
