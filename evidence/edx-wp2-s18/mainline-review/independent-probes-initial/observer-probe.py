...
results['normal']={'counts':normal,'events':o.events,'scans':o.scans}
...
for exit_code in (0,2,143):
 for failure in (False,True):
  path=OUT/f'diagnostic-{exit_code}-{failure}.json'
...
def new_error(*args,**kwargs):raise OSError(errno.EIO,'synthetic','repeat')
...
(OUT/'observer-results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2)+'\n')
