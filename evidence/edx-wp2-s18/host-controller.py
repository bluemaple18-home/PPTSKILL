...
out = repo / 'evidence/edx-wp2-s18/host-acceptance'
node = '/opt/homebrew/bin/node'
frozen = json.loads((repo / 'evidence/edx-wp2-s18/source-hashes.json').read_text())
def verify():
    result = {}
...
try:
    ...
    for key,args,limit in [
        ('deleteElement',[node,'tools/edx-wp1-s4-browser-acceptance.mjs',str(out/'delete-element'),'--delete-element-regression'],900),
    ...
    receipt['status']='PASS' if all(receipt.get(k)==0 for k in ('readinessExit','deleteElementExit','pgqExit','browserCloseExit','supervisorExit')) and receipt['ownedRootAbsent'] and receipt['isolationMarkerAbsent'] and 'after' in receipt and 'error' not in receipt else 'NOT_PASS'
