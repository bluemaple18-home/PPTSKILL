...
out = repo / 'evidence/edx-wp2-s14/host-harness-repair'
node = '/opt/homebrew/bin/node'
frozen = json.loads((repo / 'evidence/edx-wp2-s14/source-hashes.json').read_text())
def verify():
    result = {}
...
try:
    ...
    for key,args,limit in [
        ('editTextComponent',[node,'tools/edx-wp1-s4-browser-acceptance.mjs',str(out/'edit-text-component'),'--edit-text-component-regression'],900),
    ...
    receipt['status']='PASS' if all(receipt.get(k)==0 for k in ('readinessExit','editTextComponentExit','pgqExit','browserCloseExit','supervisorExit')) and receipt['ownedRootAbsent'] and receipt['isolationMarkerAbsent'] and 'after' in receipt and 'error' not in receipt else 'NOT_PASS'
