...
for(const alignment of ['left','center-x','right','top','center-y','bottom'])test('獨立：'+alignment+' revision / preserve / no-op / export',()=>{
 ...
 const expected=structuredClone(after);if(alignment.startsWith('center')){const center=(Math.min(...boxes.map(b=>b[field]))+Math.max(...boxes.map(b=>b[field]+b[size])))/2;for(const b of Object.values(expected.slides[0].composition.geometryOverrides))b[field]=Math.round(center-b[size]/2);}
 h.action('align-'+alignment);assert.deepEqual(h.getSpec(),expected);const settled=h.api.__revision();assert.equal(settled,rev+(JSON.stringify(after)===JSON.stringify(expected)?1:2));h.action('align-'+alignment);assert.equal(h.api.__revision(),settled);assert.deepEqual(h.getSpec(),expected);
 const html=h.api.exportHtml();assert.deepEqual(extractDeckSpec(html),expected);assert.doesNotMatch(html,/data-pptskill-context-toolbar|data-editor-selected="true"/);assert.deepEqual(selection(h),ids);
