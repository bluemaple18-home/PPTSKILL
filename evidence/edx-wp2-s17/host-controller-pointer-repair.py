...
out = repo / 'evidence/edx-wp2-s17/host-pointer-repair'
node = '/opt/homebrew/bin/node'
frozen = json.loads((repo / 'evidence/edx-wp2-s17/source-hashes.json').read_text())
def verify():
    result = {}
...
try:
    ...
    for key,args,limit in [
        ('textDoubleClick',[node,'tools/edx-wp1-s4-browser-acceptance.mjs',str(out/'text-double-click'),'--text-double-click-regression'],900),
    ...
    receipt['status']='PASS' if all(receipt.get(k)==0 for k in ('readinessExit','textDoubleClickExit','pgqExit','browserCloseExit','supervisorExit')) and receipt['ownedRootAbsent'] and receipt['isolationMarkerAbsent'] and 'after' in receipt and 'error' not in receipt else 'NOT_PASS'
