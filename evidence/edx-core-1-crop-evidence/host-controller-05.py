...
out = repo / 'evidence/edx-core-1-crop-evidence/host-acceptance-05'
node = '/opt/homebrew/bin/node'
frozen = json.loads((repo / 'evidence/edx-core-1-crop-evidence/source-hashes.json').read_text())
def verify():
    result = {}
...
receipt['scope'] = 'Core Crop 修後 failed PGQ 1 case＋Evidence dialog補充；15+1 unique，不是單輪16'
...
try:
    ...
    for key,args,limit in [('pgqFailed',[node,'--test','--test-concurrency=1','tests/pgq-wp4-s4-required-visibility.test.mjs'],600),('dialogVisual',[node,'evidence/edx-core-1-crop-evidence/dialog-visual-probe.mjs',str(out/'dialog')],180)]:
